...
func (uu *UserUpdate) SetNillableAccountName(s *string) *UserUpdate {
	if s != nil {
		uu.SetAccountName(*s)
	}
	return uu
...
func (uu *UserUpdate) check() error {
	...
	if v, ok := uu.mutation.AccountName(); ok {
		if err := user.AccountNameValidator(v); err != nil {
			return &ValidationError{Name: "account_name", err: fmt.Errorf(`ent: validator failed for field "User.account_name": %w`, err)}
		}
	}
	return nil
}
...
// SetNillableAccountName sets the "account_name" field if the given value is not nil.
func (uuo *UserUpdateOne) SetNillableAccountName(s *string) *UserUpdateOne {
	if s != nil {
		uuo.SetAccountName(*s)
	}
	return uuo
}
